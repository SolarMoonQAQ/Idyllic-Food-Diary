package cn.solarmoon.idyllic_food_diary.util.namespace;

public class NBTList {

    public static final String BURNING_TIME = "BurningTime";
    public static final String BURNING_TIME_SAVING = "BurningTimeSaving";
    public static final String DRAIN_TICK = "DrainTick";
    public static final String BOIL_TICK = "BoilTick";
    public static final String BREW_TICK = "BrewTick";
    public static final String SECOND_INV = "SecondInventory";
    public static final String HAS_LID = "HasLid";
    public static final String STACK = "Stack";
    public static final String SPICES = "Spices";
    public static final String CONTAINER = "Container";
    public static final String FOOD_CONTAINER = "FoodContainer";
    public static final String RESULT = "Result";
    public static final String EXP = "Experience";
    public static final String STEW_TIME = "StewTime";
    public static final String STEW_RECIPE_TIME = "StewRecipeTime";
    public static final String SIMMER_TIME = "SimmerTime";
    public static final String SIMMER_RECIPE_TIME = "SimmerRecipeTime";


    public static final String STIR_FRY_TIME = "StirFryTime";
    public static final String STIR_FRY_RECIPE_TIME = "StirFryRecipeTime";
    public static final String STIR_FRY_COUNT = "StirFryCount";
    public static final String STIR_FRY_RECIPE = "StirFryRecipe";
    public static final String STIR_FRY_STAGE_NUM = "StirFryStageNum";
    public static final String STIR_FRY_CAN = "StirFryCan";
    public static final String STIR_FRY_PENDING = "StirFryPending";

}
