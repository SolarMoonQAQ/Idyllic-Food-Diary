 ...
 * 汤锅
 */
public abstract class AbstractSoupPotEntityBlock extends BasicEntityBlock {
    ...
    public InteractionResult use(BlockState state, Level level, BlockPos pos, Player player, InteractionHand hand, BlockHitResult hitResult) {
        SoupPotBlockEntity soupPot = (SoupPotBlockEntity) level.getBlockEntity(pos);
        if(soupPot == null) return InteractionResult.PASS;
        ...
        //能够存取液体
        if (soupPot.putFluid(player, hand, false)) {
            level.playSound(null, pos, SoundEvents.BUCKET_FILL, SoundSource.PLAYERS);
            soupPot.setChanged();
            return InteractionResult.SUCCESS;
        }
        if (soupPot.takeFluid(player, hand, false)) {
        ...
        //存取任意单个物品
        if(hand.equals(InteractionHand.MAIN_HAND) && soupPot.storage(player, hand, 1, 1)) {
    ...
    public void tick(Level level, BlockPos pos, BlockState state, BlockEntity blockEntity) {
        ...
        SoupPotBlockEntity soupPot = (SoupPotBlockEntity) blockEntity;
        soupPot.tryCook();
    }
    ...
    public void animateTick(BlockState state, Level level, BlockPos pos, RandomSource random) {
        ...
        if (blockEntity instanceof SoupPotBlockEntity soupPot) {
