...
public interface IOptionalRecipeItem<T extends Recipe<RecipeWrapper>> {
    ...
    /**
     * @return 输出在匹配配方中具体选择的那个配方
     */
    T getSelectedRecipe(Entity entity);
     ...
     */
    default void recipeCheckAndUpdate(Level level, BlockHitResult hit, boolean isHeld) {
        if (isHeld) {
            BlockPos pos = hit.getBlockPos();
            ItemStack hitStack = level.getBlockState(pos).getBlock().asItem().getDefaultInstance();
            List<T> recipes = RecipeUtil.getRecipes(level, getRecipe());
