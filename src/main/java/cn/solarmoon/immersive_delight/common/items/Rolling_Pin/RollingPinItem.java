package cn.solarmoon.immersive_delight.common.items.Rolling_Pin;
...
import static cn.solarmoon.immersive_delight.common.items.Rolling_Pin.events.client.ChooseOutput.*;
...
import static cn.solarmoon.immersive_delight.particles.methods.Rolling.rolling;
...
public class RollingPinItem extends SwordItem {
    ...
    public @NotNull InteractionResult useOn(@NotNull UseOnContext context) {
        ...
        if (equalBlockPos.equals(exceptPos) || equalBlockPos.equals(exceptPos.above())) {
            ...
            List<RollingPinRecipe> recipes = GetRecipes.getRollingRecipes(context.getLevel());
        ...
        if (getUseDuration(entityLiving.getUseItem()) - entityLiving.getUseItemRemainingTicks() >= time && time > 0) {
            ...
            if(level.isClientSide) {
                ...
                if (possibleOutputs != null && !possibleOutputs.isEmpty()) {
                    output = possibleOutputs.get(currentRecipeIndex);
            ...
            if(level.isClientSide) {
                if (!actualResults.isEmpty()) {
                    for (ItemStack item : actualResults) {
